...
from scripts.detect_dead_code import (
    MethodExtractor, find_references_in_file, analyze_dead_code,
    generate_report, find_all_references, main
)
...
def test_analyze_dead_code():
    classes = {
        "UnusedClass": {
            "methods": {"method1": {"is_private": False, "is_dunder": False, "line_start": 1, "line_end": 5, "internal_calls": set()}},
            "line_start": 1, "line_end": 10
        },
        "UsedClass": {
            "methods": {
                "used_method": {"is_private": False, "is_dunder": False, "line_start": 11, "line_end": 15, "internal_calls": set()},
                "dead_private": {"is_private": True, "is_dunder": False, "line_start": 16, "line_end": 20, "internal_calls": set()},
                "__init__": {"is_private": False, "is_dunder": True, "line_start": 21, "line_end": 25, "internal_calls": set()}
            },
    ...
    with patch('scripts.detect_dead_code.BASE_DIR', mock_base):
        ...
        assert any(c["name"] == "UsedClass" for c in analysis["used_classes"])
        assert any(c["name"] == "UnusedClass" for c in analysis["unused_classes"])
...
def test_generate_report():
    ...
    assert "# Fase 3.7: Análisis de Código Muerto" in report
...
def test_main_execution():
    """main() escribe el reporte usando open; verificamos que se invoque."""
    with patch('scripts.detect_dead_code.DIALOGS_PATH') as mock_path:
        mock_path.exists.return_value = True
        # Mocking Path.relative_to globally for Path objects to avoid relativization errors
        with patch('pathlib.Path.relative_to', return_value=Path("fake.py")):
            with patch('builtins.open', mock_open(read_data="class C:\n    def m(self): pass")):
                # Mock find_all_references to return some data
                with patch('scripts.detect_dead_code.find_all_references', return_value={"C": [], "m": []}):
