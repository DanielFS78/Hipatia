...
class TestPrepStepsWidget:
    ...
    def widget(self, qtbot):
        ...
        ctrl = MagicMock(spec=["view"])
        ctrl.view = MagicMock(spec=["show_message"])
        w = PrepStepsWidget(controller=ctrl)
        qtbot.addWidget(w)
        return w
    ...
    def test_show_step_details(self, widget):
        ...
        step = {"id": 1, "nombre": "Corte", "tiempo_fase": 10, "descripcion": "Fase", "es_diario": 0, "es_verificacion": 0}
    ...
    def test_get_form_data_missing_fields(self, widget):
        """get_form_data con campos vacíos retorna None."""
        widget.show_add_new_form()
        widget.form_widgets['nombre'].setText("")
        widget.form_widgets['tiempo_fase'].setText("")
        assert widget.get_form_data() is None

    def test_get_form_data_invalid_tiempo(self, widget):
        """get_form_data con tiempo inválido retorna None."""
        widget.show_add_new_form()
        widget.form_widgets['nombre'].setText("Test")
        widget.form_widgets['tiempo_fase'].setText("abc")
        assert widget.get_form_data() is None

    def test_get_form_data_negative_tiempo(self, widget):
        """get_form_data con tiempo negativo retorna None."""
        widget.show_add_new_form()
        widget.form_widgets['nombre'].setText("Test")
    ...
    def test_on_save_button_update(self, widget, qtbot):
        ...
        step = {"id": 1, "nombre": "Corte", "tiempo_fase": 10, "descripcion": "", "es_diario": 0, "es_verificacion": 0}
        widget.show_step_details(step)
