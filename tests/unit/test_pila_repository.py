...
def test_save_and_get_all_pilas(repos, session):
    ...
    pila_id = pila_repo.save_pila(nombre='Pila 1', descripcion='Test Description', pila_de_calculo={'productos': {'P1': {'cantidad': 10}}}, production_flow=[{'task': {'name': 'Task 1'}}], simulation_results=[], producto_origen_codigo='P-ORIGIN')
...
def test_get_all_pilas_with_dates(repos, session):
    ...
    sim_results = [{'Tarea': 'T1', 'Inicio': start.isoformat(), 'Fin': end.isoformat()}]
    pila_repo.save_pila('Pila with Dates', 'Has simulation', {}, [], sim_results, 'P-DATES')
...
def test_bitacora_lifecycle(repos):
    ...
    success = pila_repo.add_diario_evento(pila_id=pila_id, fecha=fecha, dia_numero=1, plan_previsto='Plan A', trabajo_realizado='Hecho A', notas='Nota A')
    assert success is True
    ...
    pila_repo.add_diario_evento(pila_id=pila_id, fecha=fecha, dia_numero=1, plan_previsto='Plan B', trabajo_realizado='Hecho B', notas='Nota B')
...
def test_load_pila_json_errors(repos, session):
    ...
    pila_repo = repos['pila']
    pila = Pila(nombre='Corrupt Pila', descripcion='Desc', pila_de_calculo_json='{bad_json', resultados_simulacion='{bad_json', fecha_creacion=datetime.now())
    session.add(pila)
    session.commit()
    paso = PasoPila(pila_id=pila.id, orden=0, datos_paso='{bad_json')
    session.add(paso)
    session.commit()
...
def test_convert_indices_to_ids_logic(repos):
    ...
    pila_repo = repos['pila']
    flow = [{'unique_id': None, 'previous_task_index': None, 'next_cyclic_task_index': 1}, {'unique_id': None, 'previous_task_index': 0, 'next_cyclic_task_index': None}]
...
def test_convert_ids_to_indices_logic(repos):
    ...
    flow: list[dict[str, Any]] = [{'unique_id': uid1, 'next_cyclic_task_id': uid2}, {'unique_id': uid2, 'previous_task_id': uid1}]
...
def test_bitacora_get_non_existent(repos):
    ...
    pila_repo = repos['pila']
    bid, entradas = pila_repo.get_diario_bitacora(99999)
    assert bid is None
    assert entradas == []
...
def test_save_cleaning_logic(repos):
    ...
    pila_repo = repos['pila']
    flow = [{'task': {'name': 'T1', 'canvas_unique_id': 'temp1', 'original_task_id': 'orig1'}}]
...
def test_update_cleaning_logic(repos):
    ...
    flow = [{'task': {'name': 'T2', 'canvas_unique_id': 'temp2'}}]
...
def test_cyclic_index_conversion(repos):
    ...
    pila_repo = repos['pila']
    flow = [{'task': {'name': 'T0', 'id': 'uuid-0'}, 'next_cyclic_task_index': 1, 'unique_id': 'uuid-0'}, {'task': {'name': 'T1', 'id': 'uuid-1'}, 'next_cyclic_task_index': 0, 'unique_id': 'uuid-1'}]
...
def test_standard_dependency_conversion(repos):
    ...
    pila_repo = repos['pila']
    flow = [{'task': {'name': 'T0'}, 'unique_id': 'u0'}, {'task': {'name': 'T1'}, 'unique_id': 'u1', 'previous_task_index': 0}]
