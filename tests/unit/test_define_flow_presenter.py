# -*- coding: utf-8 -*-
"""Tests unitarios para DefineFlowPresenter: prepare_task_data, group_tasks, CRUD, consultas vía servicios."""
import pytest
...
class TestDefineFlowPresenter:
    ...
    def test_init(self, presenter):
        ...
        assert presenter.machine_service is None
    ...
    def test_group_tasks_zero_cycle_guard(self, presenter):
        ...
        ms = create_autospec(MachineService, instance=True)
        prep = MagicMock(spec=["get_prep_info_for_product", "get_groups_for_machine", "get_steps_for_group"])
        fab = MagicMock(spec=["get_prep_info_for_product"])
        presenter.machine_service = ms
        presenter.preparation_service = prep
        presenter.fabricacion_service = fab

        ms.get_machines_by_process_type.return_value = ["M1"]
        ...
        ms.get_machines_by_process_type.assert_called_once_with("T1")
        assert presenter.get_machines_for_task(None) == []

        prep.get_prep_info_for_product.return_value = (1, 2)
        assert presenter.get_prep_info("P1") == (1, 2)
        prep.get_prep_info_for_product.assert_called_once_with("P1")

        mock_group = MagicMock(spec=['id'])
        mock_group.id = 10
        prep.get_groups_for_machine.return_value = [mock_group]

        mock_step = MagicMock(spec=['id'])
        mock_step.id = 50
        prep.get_steps_for_group.return_value = [mock_step]

        steps = presenter.get_prep_steps_for_machine(1)
        assert len(steps) == 1
        assert steps[0].id == 50
        prep.get_groups_for_machine.assert_called_once_with(1)
        prep.get_steps_for_group.assert_called_once_with(10)

        assert presenter.get_default_step_ids(10) == [50]
        prep.get_groups_for_machine.assert_has_calls([call(1)])
        prep.get_steps_for_group.assert_has_calls([call(10), call(10)])
    ...
    def test_get_step_view_model_individual(self, presenter):
        ...
        ms = create_autospec(MachineService, instance=True)
        ms.get_all_machines.return_value = [mock_machine]
        presenter.machine_service = ms
        ...
        ms.get_all_machines.assert_called_once_with(include_inactive=True)
    ...
    def test_get_step_view_model_machine_from_service(self, presenter):
        mock_machine = MagicMock(spec=['id', 'nombre'])
        mock_machine.id = 1
    ...
    def test_get_machines_for_task_no_machine_service(self, presenter):
        presenter.machine_service = None
        task = FlowTaskDataDTO(id="T", name="T", duration=1.0, duration_per_unit=1.0, department="G")
        assert presenter.get_machines_for_task(task) == []
