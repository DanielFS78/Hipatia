...
class ProductController(QObject):
    ...
    def show_create_fabricacion_dialog(self):
        """Muestra el diálogo para crear fabricación."""
        ...
        self.logger.info("Mostrando diálogo para crear fabricación con preprocesos.")
        try:
            all_preprocesos = self.app.model.get_all_preprocesos_with_components()
            if not all_preprocesos:
                 self.view.show_message("Información", "No hay preprocesos. Cree uno antes.", "info")
                 return

            dialog = CreateFabricacionDialog(all_preprocesos, self.view)
