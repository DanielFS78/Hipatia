...
class MachineStatsManager(BaseRepository):
    ...
    def get_machine_usage_stats(self) -> List[Tuple[str, float]]:
        def _operation(session):
            result = session.query(Maquina.nombre, func.sum(Subfabricacion.tiempo).label('total_minutos')).join(
                Subfabricacion, Maquina.id == Subfabricacion.maquina_id).group_by(Maquina.nombre).order_by(
                func.sum(Subfabricacion.tiempo).desc()).all()
