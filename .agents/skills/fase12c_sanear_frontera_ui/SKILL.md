...
**Canvas de flujo (motor único, 2026-04):** el grafo interactivo de tareas vive solo en `ui/widgets/production_flow/` (`ProductionFlowCanvas`, `FlowCardWidget`, `FlowConnectionPainter`). No añadir un segundo motor bajo `ui/dialogs/`; el estado `presenter.canvas_tasks` se lee vía `core/flow_canvas_io.py` (`flow_task_entry_*`, `canvas_task_body`, etc.).
...
|------|-------|-----------------|------------------------------|--------------|
| 1 | 2026-03-20 | `ui.dialogs.card_widget` (retirado 2026-04), `workers_widget`+`worker_details_panel`, `main_window`; analizador ignora `os.environ.get` | Dict legado → `FlowTaskDataDTO.from_legacy_mapping`; asignación rápida encapsulada en panel; detección pytest con `os.getenv` | pytest (histórico): canvas_widgets; hoy `flow_canvas`, `canvas_widgets_coverage`, workers_widget, main_window; mypy en módulos tocados |
...
| 5 | 2026-03-20 | `CanvasVisualConnection` + helpers en `core/flow_canvas_io.py` (``flow_task_entry_*``); `ui/dialogs/canvas_widget.py` retirado en 2026-04 (canvas único en `ui/widgets/production_flow`) | Catálogo 355→348 (~7); `set_connections` acepta dict o DTO | Histórico: existía `test_canvas_widgets`; hoy `test_canvas_widgets_coverage` + `test_flow_canvas` |
