...
class ScheduleConfig:
    ...
    def _load_from_database(self) -> None:
        ...
        try:
            # Usar el repositorio si está disponible (Arquitectura nueva)
            if hasattr(self.db_manager, 'config_repo'):
                config_data = {
                    'work_start_time': self.db_manager.config_repo.get_setting('work_start_time', '08:00'),
                    'work_end_time': self.db_manager.config_repo.get_setting('work_end_time', '17:00'),
                    'breaks': self.db_manager.config_repo.get_setting('breaks', '[{"start": "12:00", "end": "13:00"}]'),
                    'holidays': self.db_manager.config_repo.get_setting('holidays', '[]')
                }
            else:
                # Fallback extreme (no debería ocurrir si db_manager es el nuevo)
                self.logger.error("DatabaseManager no tiene config_repo. Usando valores por defecto.")
                raise AttributeError("Missing config_repo")
    ...
    def reload_config(self, db_manager: Optional[Any] = None) -> None:
        """
    ...
    def _process_holidays(self, holidays_data: List[Any]) -> List[date]:
        """
